...
#define TIME_TO_SLEEP  10                                  /* Time ESP32 will be asleep for (in seconds) */
...
void setup() {
  ...
  if(!file) {
    ...
    writeFile(SD, "/data.txt", "Date, Day, Time, RTC Temp, Battery Voltage, Battery %, USB Connected \r\n");
  }
  else {
  ...
  int battery_precentage = map(battery,3.0,4.2,0,100); // we can map the battery voltage to a 0%-100% value
  bool usb = bdl.getVbusPresent();

  if(usb == 0){
    String usbTF = String("False"){
      else if (usb == 1){
        String usbTF = String("True")
      }
    }
  }
  ...
  String second = String(now.second(), DEC);
    ...
    dataMessage =  String(month +"/" + day +"/" + year) + "," + String(daysOfTheWeek[now.dayOfTheWeek()]) + "," + String(hour + ":" + minute + ":" + second ) + ","  + String(tempF) + "," + String(battery) + "," + String(battery_precentage)  + "," + String(usbTF) + "\r\n";
